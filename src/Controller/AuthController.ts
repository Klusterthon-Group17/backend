...
export default class AuthController {
  ...
  public async createUser(req: Request, res: Response) {
    ...
    try {
      ...
      res
        ...
        .json({ message: 'Sucess: Kindly verify your email' });
    } catch (error: any) {
      logger.error(error);
      return res
  ...
  public async authenticate(req: Request, res: Response) {
    ...
    try {
      ...
      if (error) {
        return res
          .status(StatusCodes.BAD_REQUEST)
          .json({ message: 'Invalid credentials', error });
      } else if (error) {
        return res
          .status(StatusCodes.UNAUTHORIZED)
          .json({ message: 'Email yet to be verified', error });
      }
    }
  }
  ...
  public async resetPasswordController(req: Request, res: Response) {
    ...
    try {
      const code = await this.authService.resetPasswordService(
        email,
        token,
        password
      );
